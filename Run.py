...
def main():
    ...
    for page in pages:
        ...
        _header(f"Step 1/4 — ConvertToImages  (page {p})")
        convert_pages(p)

        _header(f"Step 2/4 — SplitTableCells  (page {p})")
        ...
        _header(f"Step 3/4 — CellsToExcel     (page {p})")
        print(f"Đang xử lý trang {page}...")
        cells_to_excel(page_dir, reader, column_allowlists)

        _header(f"Step 4/4 — RefineData        (page {p})")
